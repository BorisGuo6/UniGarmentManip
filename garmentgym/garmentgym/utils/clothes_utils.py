...
def load_cloth_mesh(path):
    ...
    triangle_faces = faces  # Directly use the faces as they are already triangular

    stretch_edges, shear_edges, bend_edges = set(), set(), set()

    # Stretch edges are just the edges of each triangular face
    for face in triangle_faces:
        stretch_edges.add(tuple(sorted([face[0], face[1]])))
        stretch_edges.add(tuple(sorted([face[1], face[2]])))
        stretch_edges.add(tuple(sorted([face[2], face[0]])))

    # Shear edges do not apply to triangular faces directly, but you can leave it empty or use the same edges
    # We'll use the stretch edges to form shear edges in a triangular case if needed
    # shear_edges = stretch_edges.copy()
    ...
    return np.array(vertices), np.array(triangle_faces), \
        np.array(list(stretch_edges)), np.array(list(bend_edges)), np.array(list(shear_edges))
